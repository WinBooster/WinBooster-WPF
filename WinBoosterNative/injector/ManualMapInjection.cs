        ...
        public void CallEntrypoint(byte[] rawImage, ulong moduleHandle)
        {
            ...
            switch (TypeOfExecution)
            {
                ...
                case ExecutionType.HijackThread:
                    ...
                    Console.WriteLine($"Thread {targetThread.Id} - {targetThread.ThreadState} - {targetThread.PriorityLevel} - {targetThread.CurrentPriority}");
                    ...
                    Console.WriteLine($"{ctx.Rip.ToString("x2")} -> {remoteShellcodePointer.ToString("x2")}");
                    ...
                    // RESUME THREAD
                    Console.WriteLine("Resumed?", NT.ResumeThread(threadHandle));
                    //if ( == uint.MaxValue/*-1*/)
