...
using WinBoosterNative.injector.Extensions;
...
namespace WinBoosterNative.injector
{
    public unsafe class LoadLibraryInjection : IInjectionMethod
    {
        ...
        {
            Console.WriteLine($"Injecting {imagePath} -> {TargetProcess.ProcessName}_{TargetProcess.Id}");
            ...
            switch (TypeOfExecution)
            {
                ...
                case ExecutionType.HijackThread:
                    ...
                    Console.WriteLine($"Thread {targetThread.Id} - {targetThread.ThreadState} - {/*targetThread.WaitReason} - {targetThread.PriorityLevel} - {*/targetThread.CurrentPriority}");
                    ...
                    if (NT.SuspendThread(threadHandle) == uint.MaxValue/*-1*/)
                        Console.WriteLine($"Failed to suspend thread - {Marshal.GetLastWin32Error().ToString("x2")}");
                    ...
                    Console.WriteLine($"Stack Pointer: {ctx.Rsp.ToString("x2")} - {ctx.Rsp % 0x10}");
                    ...
                    if (NT.ResumeThread(threadHandle) == uint.MaxValue/*-1*/)
                        Console.WriteLine($"Failed to resume thread - {Marshal.GetLastWin32Error().ToString("x2")}");
        ...
        {
            Console.WriteLine($"Injecting (RAW) -> {TargetProcess.ProcessName}/{TargetProcess.Id}");
